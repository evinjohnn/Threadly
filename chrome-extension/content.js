(function() {
                ...
                <div class="threadly-content">
                    <div class="threadly-search-container">
                        <div class="threadly-search-row">
                            ...
                            <div class="threadly-saved-pill" id="threadly-saved-pill" title="Manage saved items">
                                <div class="item-left">
                            ...
                            <button id="threadly-select-btn" class="threadly-select-btn" title="Enable selection mode">
                                <span class="threadly-select-icon">
    ...
    function addEventListeners() {
        ...
        // Add event listener for saved pill
        const savedPill = document.getElementById('threadly-saved-pill');
        if (savedPill) {
            savedPill.addEventListener('click', showCollectionsManager);
        }
        
        // Add fluid search bar behavior for FAV mode
        if (searchInput) {
    ...
    async function selectFilterState(state) {
        ...
        // Update search row classes for FAV mode
        const searchRow = document.querySelector('.threadly-search-row');
        if (searchRow) {
            if (state === 'favorites') {
                searchRow.classList.add('fav-mode');
                // Trigger metaball animation for FAV mode (separated state)
                if (window.metaBallRenderer) {
                    window.metaBallRenderer.setState(2);
                }
            } else {
                searchRow.classList.remove('fav-mode');
                // Trigger metaball animation for normal mode (combined state)
                if (window.metaBallRenderer) {
                    window.metaBallRenderer.setState(1);
                }
            }
        }
    ...
    function handleSearchBlur(e) {
        if (messageFilterState === 'favorites' && searchInput.classList.contains('expanded')) {
            // Small delay to allow for focus events
            setTimeout(() => {
